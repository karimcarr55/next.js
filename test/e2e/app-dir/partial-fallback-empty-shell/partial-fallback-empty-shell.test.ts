...
describe('partial-fallback-empty-shell', () => {
  ...
  // The fixture pages render a `#rendered-at` marker (`performance.now()`,
  // prerenderable) in the deferred region. It must be re-rendered per
  // request, so it must never appear in the cached static part and must
  // never repeat across fetches: a repeated value proves a stored render is
  // being replayed from the cache. Unlike the sentinel-based assertions,
  // this observation doesn't depend on test-mode response composition.
  function readRenderedAt({
    static$,
    dynamicPart,
  }: Awaited<ReturnType<typeof fetchSplitHTML>>) {
    const inStatic = static$('#rendered-at').text()
    if (inStatic) {
      return { value: inStatic, inStatic: true }
    }
    const match = dynamicPart.match(/<div id="rendered-at">([\d.]+)<\/div>/)
    return { value: match?.[1] ?? null, inStatic: false }
  }
  ...
  describe('empty shell', () => {
    it('should keep a never-prerenderable param out of the cached shell', async () => {
      ...
      await retry(async () => {
        const first = await fetchSplitHTML(pathname)

        expect(first.static$('#id').length).toBe(0)
        expect(first.dynamicPart).toContain(`<div id="id">${id}</div>`)

        // The deferred region must be re-rendered per request: the
        // per-render marker must be absent from the static part and must
        // differ between two fetches of the same URL.
        const second = await fetchSplitHTML(pathname)
        const firstRenderedAt = readRenderedAt(first)
        const secondRenderedAt = readRenderedAt(second)

        expect(firstRenderedAt.inStatic).toBe(false)
        expect(secondRenderedAt.inStatic).toBe(false)
        expect(firstRenderedAt.value).toBeTruthy()
        expect(secondRenderedAt.value).toBeTruthy()
        expect(secondRenderedAt.value).not.toBe(firstRenderedAt.value)
      })
    })
  ...
  describe('non-empty shell', () => {
    it('should keep a never-prerenderable param out of the cached shell', async () => {
      ...
      await retry(async () => {
        ...
        expect(first.static$('#static').text()).toBe('static page content')
        expect(first.static$('#id').length).toBe(0)
        expect(first.dynamicPart).toContain(`<div id="id">${id}</div>`)
