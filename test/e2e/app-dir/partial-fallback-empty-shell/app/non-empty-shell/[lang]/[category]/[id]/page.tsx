...
async function Id({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Per-render marker (prerenderable, unlike dynamic IO), read after
  // `await params` so it belongs to the deferred region: it must be
  // re-rendered (resumed) per request and never repeat across fetches.
